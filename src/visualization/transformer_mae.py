...
Reads per-fold `test_predictions.csv` artifacts and computes:
    ...
    - Return MAE on standardized return |y_ret_true_std - y_ret_pred_std|
...
def _compute_fold_mae(pred_df: pd.DataFrame) -> dict[str, float]:
    ...
    ret_mae_std = float((pred_df["y_ret_true_std"] - pred_df["y_ret_pred_std"]).abs().mean())
    return {
        "dir_mae": dir_mae,
        "reg_mae": reg_mae,
        "ret_mae_std": ret_mae_std,
    }
...
def build_mae_tables(
    ...
    summary_df = (
        fold_df.groupby("variant", as_index=False)[["dir_mae", "reg_mae", "ret_mae_std"]]
        .mean()
        .sort_values("variant")
...
def plot_variant_mae(summary_df: pd.DataFrame, output_path: Path) -> Path:
    ...
    ax.bar(x - w, summary_df["dir_mae"], width=w, color="#E63946", label="Direction MAE")
    ax.bar(x, summary_df["reg_mae"], width=w, color="#2A9D8F", label="Regime MAE")
    ax.bar(x + w, summary_df["ret_mae_std"], width=w, color="#457B9D", label="Return MAE (std)")
    ...
    for idx, row in summary_df.iterrows():
        ax.text(x[idx] - w, row["dir_mae"] + 0.005, f"{row['dir_mae']:.3f}", ha="center", fontsize=8)
        ax.text(x[idx], row["reg_mae"] + 0.005, f"{row['reg_mae']:.3f}", ha="center", fontsize=8)
        ax.text(
            x[idx] + w,
            row["ret_mae_std"] + 0.005,
            f"{row['ret_mae_std']:.3f}",
            ha="center",
            fontsize=8,
        )
