...
def _make_dataset(n: int = N_ROWS, window: int = W, seed: int = 0) -> SPYWindowDataset:
    ...
    dir_labels = rng.integers(0, 3, n).astype(np.int64)
    ret_std = rng.standard_normal(n).astype(np.float32)
    return SPYWindowDataset(features, regime_labels, dir_labels, ret_std, window_size=window)
...
def test_dataset_length():
    ...
    # Valid windows: idx from 0 to n-W-1, so n-W total
...
def test_dataset_y_ret_is_float():
    ds = _make_dataset()
    _, _, _, y_ret = ds[0]
    assert isinstance(y_ret, float)
...
def test_dataset_windows_overlap_by_w_minus_1():
    ...
    rets = np.zeros(N_ROWS, dtype=np.float32)
    ds = SPYWindowDataset(features, regime, dirs, rets, window_size=W)
    x0, _, _, _ = ds[0]
    x1, _, _, _ = ds[1]
    # rows 1..W-1 of x0 should equal rows 0..W-2 of x1
...
def test_dataset_dir_label_from_last_window_row():
    ...
    dirs = np.arange(n, dtype=np.int64) % 3
    rets = np.zeros(n, dtype=np.float32)
    ds = SPYWindowDataset(features, regime, dirs, rets, window_size=W)
    for idx in range(len(ds)):
        _, y_dir, _, _ = ds[idx]
...
def test_dataset_last_valid_index():
    ...
    last_idx = len(ds) - 1
    x, y_dir, y_reg, y_ret = ds[last_idx]
    assert x.shape == (W, N_FEAT)
