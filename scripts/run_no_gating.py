"""Variant wrapper: run transformer training with no-gating emulation."""
...
if __name__ == "__main__":
    ...
    base_argv = _with_default_arg(base_argv, "--variant", "no_gating")
    base_argv = _with_default_arg(base_argv, "--n-heads", "4")
    # no_gating in run.py forces MASTER gate for identity-gate emulation.
    base_argv = _with_default_arg(base_argv, "--gate-mode", "master")
    base_argv = _with_default_arg(base_argv, "--output-dir", DEFAULT_OUTPUT_DIR)
    print("[Launcher] Starting no_gating training...", flush=True)
    main(argv=base_argv, default_variant="no_gating")
