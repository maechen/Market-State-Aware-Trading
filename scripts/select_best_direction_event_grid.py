...
def main() -> None:
    ...
    for fm in sorted(base.glob("k*_h*/fold_metrics.csv")):
        ...
        row["candidate"] = bool(
            row["val_coverage_weighted"] >= 0.30
            and row["val_acc_minus_majority_weighted"] > 0
            and 0.30 <= row["val_up_rate"] <= 0.70
        )
        rows.append(row)

    out = pd.DataFrame(rows)
    out["rank"] = np.nan
    eligible = out[out["eligible"]].copy().sort_values(
        ["val_dir_mae_weighted", "val_acc_minus_majority_weighted", "val_coverage_weighted", "val_mae_std"],
        ascending=[True, False, False, True],
    )
    out.loc[eligible.index, "rank"] = np.arange(1, len(eligible) + 1)

    out["candidate_rank"] = np.nan
    candidates = out[out["candidate"]].copy().sort_values(
        [
            "val_dir_mae_weighted",
            "val_acc_minus_majority_weighted",
            "val_coverage_weighted",
            "val_mae_std",
        ],
        ascending=[True, False, False, True],
    )
    out.loc[candidates.index, "candidate_rank"] = np.arange(1, len(candidates) + 1)

    out = out.drop(columns=["val_mae_std", "test_up_rate"])
    out.to_csv(base / "selection_summary.csv", index=False)
    print(out.sort_values(["rank", "candidate_rank", "val_dir_mae_weighted"], na_position="last").to_string(index=False))
